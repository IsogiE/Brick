...
Function RestorePreviousInstallLocation
  ReadRegStr $R1 SHCTX "${UNINSTKEY}" "UninstallString"
  StrCmp $R1 "" restore_previous_install_location_done 0

  ReadRegStr $4 SHCTX "${MANUPRODUCTKEY}" ""
  StrCmp $4 "" restore_previous_install_location_read_uninstall_location 0
  Call ValidatePreviousInstallLocation
  StrCmp $R2 "1" restore_previous_install_location_done 0

  restore_previous_install_location_read_uninstall_location:
  ReadRegStr $4 SHCTX "${UNINSTKEY}" "InstallLocation"
  StrCmp $4 "" restore_previous_install_location_done 0
  Call ValidatePreviousInstallLocation

  restore_previous_install_location_done:
FunctionEnd

Function ValidatePreviousInstallLocation
  StrCpy $R2 "0"
  StrCpy $0 $4 1
  ${IfThen} $0 == '"' ${|} StrCpy $4 $4 -1 1 ${|}
  IfFileExists "$4\uninstall.exe" 0 validate_previous_install_location_done
    StrCpy $INSTDIR $4
    StrCpy $R2 "1"

  validate_previous_install_location_done:
FunctionEnd
