...
App-update checks clean completed installer downloads from the temporary `Brick/updates` folder. Only installers for the running Brick version or older are removed; newer pending downloads are preserved. Locked files are retried on a later check. This cleanup never scans or removes files from WoW’s `Interface` folder.
