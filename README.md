...
Home has one **Sign in** or **Sign out** control for YouTube and Twitch. Sign in opens that provider's website inside Brick. Sign out closes its windows and clears its viewing session. These sessions are isolated by provider and Brick account and last until Brick closes. Brick does not import your normal browser's cookies; the provider handles and controls sign-in.

In **Your streams**, add a channel link or optionally authorize channel discovery, then choose what to share with the current guild. Each guild keeps its own sharing settings. YouTube channel sharing includes public and unlisted broadcasts, and you can still add an individual broadcast link. Optional channel-access credentials stay in the operating system's protected store and are managed separately from website sign-in. Signing out of a viewing session does not remove that authorization.
