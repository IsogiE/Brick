...
- The addon feed worker lives in the public Brick repo. It packages public ART source in GitHub Actions, then publishes signed public feed assets to `IsogiE/Brick-Releases`. Publishing tokens and private signing keys are stored only in GitHub Actions secrets.
